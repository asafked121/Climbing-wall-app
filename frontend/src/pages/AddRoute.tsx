...
export const AddRoute: React.FC = () => {
    ...
    useEffect(() => {
        if (zoneId !== '') {
            const fetchGrades = async () => {
                try {
                    const gradesData = await api.get<string[]>(`/routes/grades?zone_id=${zoneId}`);
    ...
    if (!user || (user.role !== 'admin' && user.role !== 'super_admin' && user.role !== 'setter')) {
        return (
            <Card>
                <h2>Access Denied</h2>
                ...
                <form onSubmit={handleSubmit} className="add-route-form">
                    <div className="input-group">
                        ...
                        >
                            ...
                            {zones.map(z => (
                                <option key={z.id} value={z.id}>{z.name} ({z.route_type})</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div className="input-group">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            ...
                            >
                                ...
                                {colors.map(c => (
                                    <option key={c.id} value={c.hex_value}>{c.name}</option>
                                ))}
                            </select>
                            <div
                                style={{
