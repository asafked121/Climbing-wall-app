...
export const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        if (zoneFilter) {
            const isValid = zones.some(z => z.route_type === filterType && z.id.toString() === zoneFilter);
    ...
    useEffect(() => {
        const fetchRoutes = async () => {
            try {
                ...
                if (canManageRoutes && statusFilter) params.append('status', statusFilter);
        ...
        <div className="dashboard">
            <div className="dashboard-header animate-fade-in">
                ...
                <div className="filter-group" style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    ...
                    >
                        ...
                        {zones.filter(z => z.route_type === filterType).map(z => (
            ...
            {isLoading ? (
                ...
                <div className="route-grid">
                    {routes.map((route) => (
                        <Link to={`/routes/${route.id}`} key={route.id} style={{ textDecoration: 'none', color: 'inherit' }}>
                            <Card className="route-card" noPadding>
                                ...
                                <div className="route-content">
                                    <div className="route-main">
                                        <h3 className="route-name">{(route.color_name || route.color).charAt(0).toUpperCase() + (route.color_name || route.color).slice(1)} Route</h3>
