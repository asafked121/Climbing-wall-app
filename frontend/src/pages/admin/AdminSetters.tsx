...
interface Setter { id: number; name: string; is_active: boolean; }
...
export const AdminSetters: React.FC = () => {
    ...
    const fetchSetters = async () => {
        try { const data = await api.get<Setter[]>('/admin/setters'); setSetters(data); } catch (err) { console.error(err); }
    };
    ...
    const handleAddSetter = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!newSetterName.trim()) { setError('Setter name required'); return; }
        setIsLoadingSetters(true); setError('');
        try {
            await api.post('/admin/setters', { name: newSetterName.trim(), is_active: true });
            setNewSetterName(''); fetchSetters();
        } catch (err: any) { setError(err.message || 'Failed to add setter'); } finally { setIsLoadingSetters(false); }
    };

    const handleDeleteSetter = async (setterId: number) => {
        if (!window.confirm('Are you sure you want to delete this setter?')) return;
        try { await api.delete(`/admin/setters/${setterId}`); fetchSetters(); } catch (err: any) { setError(err.message); }
    };
    ...
    return (
        <div className="admin-dashboard animate-fade-in" style={{ padding: '1rem' }}>
            ...
            <Card>
                ...
                <form onSubmit={handleAddSetter} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end', marginBottom: '2rem', flexWrap: 'wrap' }}>
                    ...
                    <Button type="submit" isLoading={isLoadingSetters} style={{ marginBottom: '2px' }}>
                        Add Setter
                    </Button>
                </form>
                ...
                <div className="ios-list-container">
                    {setters.map((setter, index) => (
                        <div key={setter.id} className={`ios-list-item ${index === setters.length - 1 ? 'last-item' : ''}`}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                <div>
                                    <div className="ios-list-grade" style={{ color: 'var(--text-primary)' }}>{setter.name}</div>
                                </div>
                            </div>
                            <Button
                                variant="danger"
