...
function baseBarOptions(horizontal = false): object {
    return {
        ...
        scales: {
            ...
            y: {
                grid: { color: getGridColor(), drawBorder: false },
                ticks: { color: getTextColor(), font: { family: 'Outfit' }, precision: 0 },
                beginAtZero: true,
            },
        },
    };
...
export const AnalyticsDashboard: React.FC = () => {
    ...
    const totalAscents = data.ascents_by_grade.reduce((sum, g) => sum + g.count, 0);
    const totalRatings = data.rating_distribution.reduce((sum, r) => sum + r.count, 0);
    ...
    return (
        <div className="analytics-dashboard animate-fade-in">
            ...
            <p className="analytics-subtitle">Performance insights &amp; climbing data</p>
            ...
            <div className="analytics-filter-bar">
                <div className="analytics-filter-group">
                    ...
                    <div className="analytics-segmented">
                        {([['', 'All'], ['active', 'Active'], ['archived', 'Archived']] as const).map(([val, label]) => (
                            <button
                                key={val}
                ...
                <div className="analytics-filter-group">
                    ...
                    <div className="analytics-segmented">
                        {([['', 'All'], ['boulder', 'Boulder'], ['top_rope', 'Top Rope']] as const).map(([val, label]) => (
                            <button
                                key={val}
                ...
                {(statusFilter !== 'active' || routeTypeFilter || dateFrom || dateTo) && (
                    <button
                        className="analytics-clear-btn"
                        onClick={() => { setStatusFilter('active'); setRouteTypeFilter(''); setDateFrom(''); setDateTo(''); }}
                    >
                        Reset
                    </button>
                )}
            ...
            <div className="analytics-grid">
                ...
                <div className="analytics-chart-card">
                    ...
                    <div className="analytics-chart-container">
                        <Bar data={gradeDistChartData} options={baseBarOptions(true) as object} />
                    </div>
                </div>
                ...
                <div className="analytics-chart-card">
                    ...
                    <div className="analytics-chart-container">
                        <Bar data={ascentsByGradeData} options={baseBarOptions() as object} />
                    </div>
                </div>
                            ...
                            <tbody>
                                {data.top_rated_routes.map((route, idx) => (
                                    <tr key={route.route_id}>
                                        ...
                                        <td>{route.color}</td>
                                        <td>
                                            <span className="stars-display">{route.avg_rating.toFixed(1)} ★</span>
                                            <span className="rating-count-badge">({route.rating_count})</span>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
