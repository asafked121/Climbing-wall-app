...
export const Register: React.FC = () => {
    ...
    const handleFinalSubmit = async (e: React.FormEvent) => {
        ...
        if (!acceptedTos) {
            setError('You must accept the Terms of Service and Assumption of Risk to register.');
            return;
        }
        ...
        try {
            ...
            if (err.response?.status === 422 && err.response?.data?.detail?.[0]?.msg?.includes('13 years old')) {
                setIsAgeBlocked(true);
                localStorage.setItem('age_gate_failed', 'true');
            } else {
        ...
        return (
            <Card>
                <div style={{ textAlign: 'center', padding: '2rem 1rem' }}>
                    <h2 style={{ marginBottom: '1rem', color: 'var(--danger-color)' }}>Registration Unavailable</h2>
                    <p style={{ color: 'var(--text-secondary)' }}>
                        To protect your privacy, you cannot create an account at this time.
                        Please ask your parent or legal guardian to create an account for you.
                    </p>
                    <div style={{ marginTop: '2rem' }}>
                        <Link to="/" style={{ color: 'var(--primary-color)', textDecoration: 'none', fontWeight: 'bold' }}>
                            Return to Home
                        </Link>
                    </div>
                </div>
    ...
    return (
        <Card>
            ...
            {error && (
                <div style={{ color: 'var(--danger-color)', marginBottom: '1rem', fontSize: '0.875rem', textAlign: 'center', background: 'rgba(239, 68, 68, 0.1)', padding: '0.5rem', borderRadius: 'var(--radius-sm)' }}>
                    {error}
                </div>
            )}
            ...
            {step === 1 ? (
                ...
                <form onSubmit={handleFinalSubmit}>
                    ...
                    <div style={{ marginTop: '1.5rem', display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                        ...
                        />
                        <label htmlFor="tos" style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                            I agree to the <Link to="/terms" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary-color)' }}>Terms of Service</Link> and Privacy Policy. I acknowledge that climbing is an inherently dangerous sport and assume all risks associated.
                        </label>
                    </div>
