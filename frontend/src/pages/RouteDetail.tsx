...
export const RouteDetail: React.FC = () => {
      ...
      const fetchGrades = async () => {
        try {
          const gradesData = await api.get<string[]>(
            `/routes/grades?zone_id=${route.zone_id}`,
          );
  ...
  useEffect(() => {
    ...
    const fetchEditGrades = async () => {
      try {
        const gradesData = await api.get<string[]>(
          `/routes/grades?zone_id=${editZoneId}`,
        );
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editGrade && editGrade !== route.intended_grade)
        updatePayload.intended_grade = editGrade;
      if (editSetterId !== "" && editSetterId !== route.setter?.id)
        updatePayload.setter_id = editSetterId;
      if (editSetDate && editSetDate !== route.set_date?.split("T")[0])
        updatePayload.set_date = editSetDate;
      if (editStatus && editStatus !== route.status)
  ...
  const handleToggleArchive = async () => {
    ...
    try {
      setIsSubmitting(true);
      await api.patch(`/admin/routes/${route.id}/archive`, {
        status: newStatus,
      });
      await fetchRouteDetail(false);
    } catch (err: any) {
  ...
  const handleUnlogAscent = async (ascentType: string) => {
    ...
    const ascentRecord = route.ascents.find(
      (a) => a.user_id === user.id && a.ascent_type === ascentType,
    );
  ...
  // Calculations
  const avgRating =
    route.route_ratings.length > 0
      ? (
          route.route_ratings.reduce((acc, r) => acc + r.rating, 0) /
          route.route_ratings.length
        ).toFixed(1)
      : "No ratings yet";
  // Ascent Counts
  const boulderAscents = route.ascents.filter(
    (a) => a.ascent_type === "boulder",
  ).length;
  const topRopeAscents = route.ascents.filter(
    (a) => a.ascent_type === "top_rope",
  ).length;
  const leadAscents = route.ascents.filter(
    (a) => a.ascent_type === "lead",
  ).length;

  const hasLoggedAscent = (type: string) => {
    if (!user) return false;
    return route.ascents.some(
      (a) => a.user_id === user.id && a.ascent_type === type,
    );
  };
  ...
  const currentUserVote = user
    ? route.grade_votes.find((v) => v.user_id === user.id)
    : null;
  ...
  return (
    <div className="route-detail-page animate-fade-in">
            ...
            <div className="edit-form-grid">
              ...
              <div className="input-group">
                ...
                <select
                  ...
                  value={editSetterId}
                  onChange={(e) =>
                    setEditSetterId(Number(e.target.value) || "")
                  }
                >
              ...
              <div className="input-group">
                ...
                <input
                  ...
                  className="input-field"
                  value={editSetDate}
                  onChange={(e) => setEditSetDate(e.target.value)}
                />
              </div>
              <div className="input-group">
      ...
      <Card className="detail-card">
        {route.photo_url && (
          <div
            className="route-photo-container"
            style={{
        ...
        <div
          ...
          style={{ backgroundColor: route.color || "var(--primary-color)" }}
        />

        <div className="detail-header">
          <h1 className="detail-title">
            {(route.color_name || route.color).charAt(0).toUpperCase() +
              (route.color_name || route.color).slice(1)}{" "}
            Route in {route.zone?.name || "Unknown Zone"}
          </h1>
        ...
        <div className="detail-meta">
          <p>
            <strong>Setter:</strong> {route.setter?.name || "Unknown"}
          </p>
          <p>
          ...
          <p>
            ...
            <span style={{ textTransform: "capitalize" }}>{route.status}</span>
          </p>
          <p>
            <strong>Community Rating:</strong> ⭐ {avgRating} (
            {route.route_ratings.length} ratings)
          </p>
          {route.zone?.route_type === "boulder" ? (
            <p>
              <strong>Total Ascents:</strong> {boulderAscents}
            </p>
          ) : (
            <p>
              <strong>Ascents:</strong> {topRopeAscents} Top Rope{" "}
              {isLeadAllowed && `/ ${leadAscents} Lead`}
            </p>
          )}
        </div>
      </Card>
          ...
          <Card>
            ...
            <div className="vote-group">
              ...
              <Button
                ...
                disabled={isSubmitting || !votedGrade}
              >
            ...
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
              {route.zone?.route_type === "boulder" ? (
                hasLoggedAscent("boulder") ? (
                  <Button
                    variant="secondary"
                    onClick={() => handleUnlogAscent("boulder")}
                    disabled={isSubmitting}
                  >
                    Unlog Boulder
                  </Button>
                  ...
                  <Button
                    variant="primary"
                    onClick={() => handleLogAscent("boulder")}
                    disabled={isSubmitting}
                  >
                    Log Boulder
                  </Button>
                ...
                <>
                  {hasLoggedAscent("top_rope") ? (
                    <Button
                      variant="secondary"
                      onClick={() => handleUnlogAscent("top_rope")}
                      disabled={isSubmitting}
                    >
                      Unlog Top Rope
                    </Button>
                    ...
                    <Button
                      variant="primary"
                      onClick={() => handleLogAscent("top_rope")}
                      disabled={isSubmitting}
                    >
                      Log Top Rope
                    </Button>
                  ...
                  {isLeadAllowed &&
                    (hasLoggedAscent("lead") ? (
                      <Button
                        variant="secondary"
                        onClick={() => handleUnlogAscent("lead")}
                        disabled={isSubmitting}
                      >
                        Unlog Lead
                      </Button>
      ...
      <div className="interactions-grid">
        ...
        <Card>
          ...
          {route.grade_votes.length === 0 ? (
            ...
            <div
              className="consensus-bar-container"
              style={{ padding: "0.5rem 0" }}
            >
              {/* Group votes by grade and sort by most popular */}
              {(() => {
                const voteCounts = route.grade_votes.reduce(
                  (acc, v) => {
