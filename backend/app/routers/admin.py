...
from app.dependencies import get_current_admin, get_current_setter_or_admin, get_current_super_admin
...
def update_user_ban_status(user_id: int, ban_update: schemas.UserBanUpdate, db: Session = Depends(get_db), current_admin: models.User = Depends(get_current_admin)):
...
def update_user_role(user_id: int, role_update: schemas.UserRoleUpdate, db: Session = Depends(get_db), current_admin: models.User = Depends(get_current_admin)):
    db_user = db.query(models.User).filter(models.User.id == user_id).first()
    if not db_user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    if current_admin.role == "admin":
        if role_update.role in ["admin", "super_admin"] or db_user.role in ["admin", "super_admin"]:
             raise HTTPException(status_code=403, detail="Admins can only manage students and setters")
...
def delete_user(user_id: int, db: Session = Depends(get_db), current_super_admin: models.User = Depends(get_current_super_admin)):
    db_user = db.query(models.User).filter(models.User.id == user_id).first()
    if not db_user:
        raise HTTPException(status_code=404, detail="User not found")
...
def create_zone(zone: schemas.ZoneCreate, db: Session = Depends(get_db), current_admin: models.User = Depends(get_current_admin)):
    db_zone = models.Zone(name=zone.name, description=zone.description, route_type=zone.route_type)
    db.add(db_zone)
    db.commit()
    db.refresh(db_zone)
    return db_zone
...
@router.post("/colors", response_model=schemas.ColorResponse, status_code=status.HTTP_201_CREATED)
def create_color(color: schemas.ColorCreate, db: Session = Depends(get_db), current_admin: models.User = Depends(get_current_admin)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="Color with this name might already exist")
    return db_color

@router.delete("/colors/{color_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_color(color_id: int, db: Session = Depends(get_db), current_admin: models.User = Depends(get_current_admin)):
...
@router.post("/setters", response_model=schemas.SetterResponse, status_code=status.HTTP_201_CREATED)
def create_setter(setter: schemas.SetterCreate, db: Session = Depends(get_db), current_admin: models.User = Depends(get_current_admin)):
...
@router.post("/routes", response_model=schemas.RouteResponse, status_code=status.HTTP_201_CREATED)
def create_route(route: schemas.RouteCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_setter_or_admin)):
    zone = db.query(models.Zone).filter(models.Zone.id == route.zone_id).first()
    if not zone:
        raise HTTPException(status_code=404, detail="Zone not found")
        
    if route.setter_id is not None:
        setter = db.query(models.Setter).filter(models.Setter.id == route.setter_id).first()
        if not setter:
            raise HTTPException(status_code=404, detail="Setter not found")
...
def update_route(route_id: int, route_update: schemas.RouteUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_setter_or_admin)):
    db_route = db.query(models.Route).filter(models.Route.id == route_id).first()
    if not db_route:
        raise HTTPException(status_code=404, detail="Route not found")
        
    if route_update.zone_id is not None:
        zone = db.query(models.Zone).filter(models.Zone.id == route_update.zone_id).first()
        if not zone:
            raise HTTPException(status_code=404, detail="Zone not found")
        db_route.zone_id = route_update.zone_id
        
    if route_update.setter_id is not None:
        setter = db.query(models.Setter).filter(models.Setter.id == route_update.setter_id).first()
        if not setter:
            raise HTTPException(status_code=404, detail="Setter not found")
    ...
    if route_update.intended_grade is not None:
        zone = db.query(models.Zone).filter(models.Zone.id == (route_update.zone_id or db_route.zone_id)).first()
        if zone:
...
def archive_route(route_id: int, archive_data: schemas.RouteArchive, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_setter_or_admin)):
    db_route = db.query(models.Route).filter(models.Route.id == route_id).first()
    if not db_route:
        raise HTTPException(status_code=404, detail="Route not found")
...
def get_bulk_route_template(
    ...
    if zones:
        dv_zone = DataValidation(type="list", formula1=f"Reference!$A$2:$A${len(zones) + 1}", allow_blank=False)
    ...
    if setters:
        ...
        dv_setter = DataValidation(type="list", formula1=f"Reference!$B$2:$B${none_row}", allow_blank=True)
    ...
    if colors:
        dv_color = DataValidation(type="list", formula1=f"Reference!$C$2:$C${len(colors) + 1}", allow_blank=False)
    ...
    if all_grades:
        dv_grade = DataValidation(type="list", formula1=f"Reference!$D$2:$D${len(all_grades) + 1}", allow_blank=False)
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=route_import_template.xlsx"}
    )
...
def bulk_upload_routes(
    ...
    expected_headers = ["zone_name", "setter_name", "color_name", "intended_grade", "set_date"]
    ...
    if len(data_rows) > 500:
        raise HTTPException(status_code=400, detail="Maximum 500 rows allowed per upload")
    ...
    setters = {s.name.lower(): s for s in db.query(models.Setter).filter(models.Setter.is_active == True).all()}
    ...
    for i, row in enumerate(data_rows):
        ...
        zone = None
        if not zone_name:
            errors.append(schemas.BulkUploadRowError(row=row_num, field="zone_name", message="Required"))
            row_has_error = True
        else:
            zone = zones.get(zone_name.lower())
            if not zone:
                errors.append(schemas.BulkUploadRowError(row=row_num, field="zone_name", message=f"Zone '{zone_name}' not found"))
                row_has_error = True
        ...
        if setter_name and setter_name.lower() != 'none':
            ...
            if not setter:
                errors.append(schemas.BulkUploadRowError(row=row_num, field="setter_name", message=f"Setter '{setter_name}' not found or inactive"))
                row_has_error = True
            else:
                setter_id = setter.id
        ...
        color_hex = None
        if not color_name:
            errors.append(schemas.BulkUploadRowError(row=row_num, field="color_name", message="Required"))
            row_has_error = True
        else:
            color = colors.get(color_name.lower())
            if not color:
                errors.append(schemas.BulkUploadRowError(row=row_num, field="color_name", message=f"Color '{color_name}' not found"))
                row_has_error = True
            else:
        ...
        if not intended_grade:
            errors.append(schemas.BulkUploadRowError(row=row_num, field="intended_grade", message="Required"))
            row_has_error = True
        elif zone:
            valid_grades = GRADES_BY_TYPE.get(zone.route_type, [])
            if intended_grade not in valid_grades:
                errors.append(schemas.BulkUploadRowError(row=row_num, field="intended_grade", message=f"Grade '{intended_grade}' invalid for {zone.route_type} zone"))
                row_has_error = True
        ...
        if set_date_raw:
            try:
                ...
                if isinstance(row[header_indices["set_date"]], date):
                    parsed_date = row[header_indices["set_date"]].date() if hasattr(row[header_indices["set_date"]], 'date') else row[header_indices["set_date"]]
                else:
                    parsed_date = date.fromisoformat(set_date_raw.split("T")[0])
            except ValueError:
                errors.append(schemas.BulkUploadRowError(row=row_num, field="set_date", message="Invalid format. Use YYYY-MM-DD"))
                row_has_error = True
...
def delete_comment(comment_id: int, db: Session = Depends(get_db), current_admin: models.User = Depends(get_current_admin)):
